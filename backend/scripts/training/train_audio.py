...
for sample in tqdm(raw, desc="Processing audio"):
    try:
        ...
        if isinstance(audio_col, dict):
            if "bytes" in audio_col and audio_col["bytes"] is not None:
                arr, sr = sf.read(io.BytesIO(audio_col["bytes"]))
            elif "path" in audio_col and audio_col["path"] is not None:
                arr, sr = sf.read(audio_col["path"])
            elif "array" in audio_col and audio_col["array"] is not None:
